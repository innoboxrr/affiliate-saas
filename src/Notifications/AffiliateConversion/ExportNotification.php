...
namespace Innoboxrr\AffiliateSaas\Notifications\AffiliateConversion;

use Maatwebsite\Excel\Facades\Excel;
use Innoboxrr\AffiliateSaas\Exports\AffiliateConversionsExports;
...
class ExportNotification extends Notification
{
    ...
    {
        return config('innoboxrraffiliatesaas.notification_via', ['mail', 'database']);
    }
    ...
    {
        Excel::store(
            new AffiliateConversionsExports($this->data), 
            $this->path, 
            config('innoboxrraffiliatesaas.export_disk', 's3')
        );
    }

    private function getSubject()
    {
        return env('APP_NAME') . ' | AffiliateConversion export notification';
    }
