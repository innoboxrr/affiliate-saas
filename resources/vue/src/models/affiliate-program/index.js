...
export const API_ROUTE_PREFIX = 'api.innoboxrr.affiliatesaas.affiliate_program.'; // Reemplaza con la ruta adecuada
	...
	return [
		{
			id: 'create',
			name: 'Create',
			callback: 'createAffiliateProgram',
			icon: 'fa-plus',
			route: true,
			policy: false,
			params: {
...
export const dataTableHead = () => {
	return [
		...
		},
		{
			...
			html: true,
			parser: (value) => {
				return `<span class="block max-w-[300px] truncate text-gray-700 dark:text-gray-300" title="${value}">${value}</span>`;
			}
		},
		{
			id: 'workspace_id',
